...
abstract class WebTestCase extends BaseWebTestCase
{
    ...
    public function loadFixtureFiles(array $paths = array(), $append = false, $omName = null, $registryName = 'doctrine', $purgeMode = null)
    {
        if (!class_exists('Nelmio\Alice\Fixtures') && !(class_exists('Nelmio\Alice\FixtureSet') && class_exists('Fidry\AliceDataFixtures\Loader\PersisterLoader'))) {
            ...
            // @codeCoverageIgnoreStart
            throw new \BadMethodCallException('nelmio/alice v2.*.* OR (nelmio/alice v3.*.* vs theofidry/AliceDataFixtures) should be installed to use this method');
            // @codeCoverageIgnoreEnd
        }
        ...
        if (class_exists('Nelmio\Alice\Fixtures')) {
            ...
            if ($container->has($fidryLoaderServiceName)) {
                return $container->get($fidryLoaderServiceName)->load($files);
            } else {
                throw new \BadMethodCallException('please add $bundles[] = new Fidry\AliceDataFixtures\Bridge\Symfony\FidryAliceDataFixturesBundle(); to AppKernel');
            }
        }
    }
